...
public class NumInMass {
    ...
    void checkArray(int []arr,int size)
    {
        ...
        for(int i = 0; i < size; i++)
        {
            if(number == arr[i])
            {
                System.out.println("Ur number exists in array");
                break;
            }
            else if((arr[i] != number) && (i == size-1))
            {
                System.out.println("Ur number doesn't exist in array");
            }
        }
    }
